...
{
    using CrudEntity = Menu;
    using EntityDto = MenuDto;

    public class MenuService : IMenuService, IService
    {
        private readonly ILogger _logger;
        private readonly IMapper _mapper;
        private readonly IUnitOfWork _unitOfWork;

        private const string _entityName = nameof(Menu);
        private readonly DbSet<CrudEntity> _entities;

        public MenuService(ILogger<MenuService> logger, IMapper mapper, IUnitOfWork unitOfWork)
        {
        ...
        {
            var query = _entities.Include(i => i.Business).AsQueryable();
        ...
        {
            var entity = await _entities.Include(i => i.Business).FirstOrDefaultAsync(x => x.Id == id, ct);
            return _mapper.Map<EntityDto>(entity);
        }
        ...
        public async Task<bool> IsDuplicateNameAsync(string name, Guid? dinerId = null, Guid? id = null)
        {
            ...
            return await _entities.AnyAsync(x =>
                x.Name == name &&
                (!dinerId.HasValue || x.BusinessId == dinerId.Value) &&
